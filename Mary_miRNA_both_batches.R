...
length(unique(mary_data$Sample_ID)) # 67 this is right - with the one removed that Mary said
...
normaility_check$p.value <- as.numeric(normaility_check$p.value)
normaility_check$normal[normaility_check$p.value < 0.05] <- "No"
table(normaility_check$normal) # 6 normal, 57 gamma
...
length(unique(normal_miRNAs$miRNA)) # 6
...
length(unique(gamma_miRNAs$miRNA)) # 57
...
write.table(significant, file="NEW_sig_interaction_miRNAs.txt", sep='\t', quote = F,
...
#hsa-miR-17-5p (interaction: but all lost in posthoc) 
...
anova(m)
EMM <- emmeans(m, ~ Condition*Sex, adjust="tukey", mode="linear.predictor", type="Score")
pairs(EMM)
...
int_sig <- as.data.frame(c("hsa-miR-143-3p"))
...
# Remove ones which have interaction (actually don't)
length(unique(normal_miRNAs$miRNA)) #6
#normal_miRNAs <- normal_miRNAs[!(normal_miRNAs$miRNA=="hsa-miR-143-3p" |
#                                 normal_miRNAs$miRNA=="hsa-miR-17-5p"),]
...
length(unique(gamma_miRNAs$miRNA)) #52
...
write.table(significant, file="NEW_sig_condition_miRNAs.txt", sep='\t', quote = F,
            col.names = T, row.names = F)
...
# Look at the other replicate of 103 as well to see
#hsa-miR-103a-3p...82
...
length(unique(significant$miRNA)) # 5 miRNAs have something going on
write.table(significant, file="NEW_sig_sex_miRNAs.txt", sep='\t', quote = F,
            col.names = T, row.names = F)

# Pull out and plot sex significant, one is interaction and is already accounted for
...
sex_sig <- as.data.frame(c("hsa-miR-423-5p","hsa-miR-451a","hsa-miR-17-3p",
                           "hsa-miR-17-5p"))
